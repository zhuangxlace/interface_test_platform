...
# id 任何表的id都不需要我们亲自写，都是自动生成并且自增 主键不唯一
# 括号内就是约束条件。CharField 是字符串。DateTimeField是时间。
# max_length是最大允许长度，null=True ，是允许为Null, auto_now ,是自动填入时间无需我们手动填入了。
...
class DB_home_href(models.Model):
    ...
    href = models.CharField(max_length=2000, null=True)  # 超链接url
...
class DB_project(models.Model):
    ...
    user = models.CharField(max_length=100,null=True)  # 项目创建者的名字
...
class DB_cases(models.Model):
    ...
    name = models.CharField(max_length=10, null=True)  # 用例名字
...
class DB_step(models.Model):
    ...
    api_header = models.CharField(max_length=1000, null=True)  # 请求头
    api_body_method = models.CharField(max_length=30, null=True)  # 请求体编码类型
    api_body = models.CharField(max_length=1000, null=True)  # 请求体
